...
class User {
	...
	function userAuthentication_2($USERNAME,$h_pass){
		global $mydb;
		...
		//	$_SESSION['USERID']   		= '1001000110110';
		 //	$_SESSION['FULLNAME']      	= 'Programmer';
			...
			$mydb->setQuery("SELECT * FROM `tblemployees` WHERE `EMPUSERNAME` = '". $USERNAME ."' and `EMPPASSWORD` = '". $h_pass ."'");
			$cur = $mydb->executeQuery();
			if($cur==false){
				die(mysql_error());
			}
			$row_count = $mydb->num_rows($cur);//get the number of count
			 if ($row_count == 1){
			 	$user_found = $mydb->loadSingleResult();
			 	$_SESSION['USERID']   		= $user_found->EMPLOYEEID;
				$_SESSION['FULLNAME']      	= $user_found->FNAME;
			  	$_SESSION['USERNAME'] 		= $user_found->LNAME;
			 	$_SESSION['PASS'] 			= $user_found->EMPPASSWORD;
			 	$_SESSION['ROLE'] 			= $user_found->POSITION;
				$_SESSION['PICLOCATION'] 	= $user_found->EMPPHOTO;
		        $_SESSION['COMPANY_ID'] 	= $user_found->COMPANYID;
